...
describe MagicSeven do
  ...
  describe '#play' do
    context 'when the random number is 3' do
      ...
      subject(:game_one) { described_class.new(3) }

      it 'will return 7' do
        expect(game_one.play).to eq(7)
      end
    end

    context 'when the random number is ...' do
      # remove the 'x' before running this test
      subject(:game_two) { described_class.new(5) }

      it 'will return 7' do
        expect(game_two.play).to eq(7)
      end
    end

    context 'when the random number is ...' do
      # remove the 'x' before running this test
      subject(:game_three) { described_class.new(7) }

      it 'will return 7' do
        expect(game_three.play).to eq(7)
      end
    end
  end
end
