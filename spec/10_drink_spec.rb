...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_drink) { described_class.new('horchata', 16) }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_drink.type).to eq('horchata')
      end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_rbv) { described_class.new('rbv') }
      # remove the 'x' before running this test
      it 'is full' do
        expect(my_rbv).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_espresso) { described_class.new('rbv', 2) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_espresso).not_to be_full
      end
    end
  end
end
