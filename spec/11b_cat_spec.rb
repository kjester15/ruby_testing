...
describe Cat do
  ...
  subject(:boofers) { described_class.new('Boofers', 'Big ol Boy', 'white') }
  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'when cat responds to meow' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not hungry' do
    expect(boofers).not_to be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(boofers).to be_hiding
  end
end
